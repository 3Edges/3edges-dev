resource "kubernetes_secret" "configuration_secrets" {
  ...
  data = {
    ...
    DB_PASSWORD         = var.three_edges_secret_DB_PASSWORD
...
resource "kubernetes_secret" "dataloader_secrets" {
  ...
  data = {
    dbPass          = var.three_edges_secret_DB_PASSWORD
    OIDC_CLIENT_PWD = var.shared_secret_OIDC_CLIENT_PWD
  }
...
resource "kubernetes_secret" "idp_secrets" {
  ...
  data = {
    ...
    DB_PASSWORD              = var.three_edges_secret_DB_PASSWORD
    CLIENT_PWD_3EDGES        = var.shared_secret_OIDC_CLIENT_PWD
    PRIM_UI_CLIENT_PWD       = var.shared_secret_OIDC_CLIENT_PWD
    INTERNAL_SECRET          = var.shared_secret_INTERNAL_SECRET
  }
...
resource "kubernetes_secret" "ui_secrets" {
  ...
  data = {
    REACT_APP_OIDC_CLIENT_PWD      = var.shared_secret_OIDC_CLIENT_PWD
    REACT_APP_INTERNAL_SECRET      = var.shared_secret_INTERNAL_SECRET
