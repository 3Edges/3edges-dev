...
data "http" "query_3" {
  ...
  request_body = jsonencode(
    {
      "statement" : "MERGE (n:SubscribtionLevel) SET n = { Throttling: 50000, isVPC: false, DataLoader: false, MaxDataPassthrough: 1, MaxUsers: 1, Support: \"0\", Uptime: 0, MaxAPI: 1, MaxOrgs: 0, name: \"FREE\", isCluster: false, isAuthZ: true } RETURN n;"
    }
  )

  depends_on = [data.http.query_1, data.http.query_2]
}
...
data "http" "query_4" {
  ...
  request_body = jsonencode(
    {
      "statement" : "MERGE (n:SubscribtionLevel) SET n = { Throttling: 50000, isVPC: false, DataLoader: true, MaxDataPassthrough: 5, MaxUsers: 99, Support: \"Unlimited\", Uptime: \"8x5\", MaxAPI: 4, MaxOrgs: 1, name: \"PRO\", isCluster: true, isAuthZ: true } RETURN n;"

    }
  )

  depends_on = [data.http.query_1, data.http.query_2, data.http.query_3]
}
...
data "http" "query_5" {
  ...
  request_body = jsonencode(
    {
      "statement" : "MERGE (n:SubscribtionLevel) SET n = { Throttling: 50000, isVPC: true, MaxDataPassthrough: \"Unlimited\", DataLoader: true, Uptime: 99.95, MaxUsers: \"Unlimited\", Support: \"24x7\", MaxOrgs: \"Unlimited\", MaxAPI: \"Unlimited\", name: \"ENT\", isCluster: true, isAuthZ: true } RETURN n;"
    }
  )

  depends_on = [data.http.query_1, data.http.query_2, data.http.query_3, data.http.query_4]
}
